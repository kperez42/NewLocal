...
//  NewLocal
//
//  Connections view with three tabs: Want to Connect, My Requests, Connections
//
...
struct LikesView: View {
    ...
    private let tabs = ["Want to Connect", "My Requests", "Connections"]
                            ...
                            if viewModel.mutualLikes.count > 0 {
                                ...
                                HStack(spacing: 4) {
                                    Image(systemName: "person.2.fill")
                                        .font(.caption)
                                    Text("\(viewModel.mutualLikes.count) connected")
                                        .fontWeight(.semibold)
                                }
                            }
                        }
    ...
    private var premiumUnlockCard: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                Text("\(viewModel.usersWhoLikedMe.count) people want to connect!")
                    .font(.title2)
                    .fontWeight(.bold)
    ...
    private var premiumFeaturesPreview: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                premiumFeatureRow(icon: "eye.fill", title: "See Who Wants to Connect", description: "Connect instantly with people interested in meeting you", color: .teal)
                premiumFeatureRow(icon: "infinity", title: "Unlimited Connections", description: "No daily limits, reach out to as many people as you want", color: .blue)
                premiumFeatureRow(icon: "star.fill", title: "Advanced Filters", description: "Filter by profession, interests, neighborhood and more", color: .orange)
            }
        }
        .padding(20)
        .background(
        ...
        Group {
            if viewModel.usersILiked.isEmpty {
                emptyStateView(
                    ...
                    message: "Browse the community and send connection requests to people you'd like to meet!"
                )
            } else {
        ...
        Group {
            if viewModel.mutualLikes.isEmpty {
                emptyStateView(
                    ...
                    message: "When you and someone else both want to connect, you'll see them here!"
                )
            } else {
